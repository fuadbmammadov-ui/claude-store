...
async function computeTotals(session) {
  ...
  const [
    ...
    prisma.sale.aggregate({ _sum: { paidAmount: true }, where: { paymentType: 'CASH', createdAt: { gte: from, lte: to }, voided: false } }),
    prisma.sale.aggregate({ _sum: { paidAmount: true }, where: { paymentType: 'CARD', createdAt: { gte: from, lte: to }, voided: false } }),
    prisma.sale.aggregate({ _sum: { paidAmount: true }, where: { paymentType: 'TRANSFER', createdAt: { gte: from, lte: to }, voided: false } }),
    prisma.sale.aggregate({ _sum: { totalAmount: true }, where: { paymentType: 'DEBT', createdAt: { gte: from, lte: to }, voided: false } }),
